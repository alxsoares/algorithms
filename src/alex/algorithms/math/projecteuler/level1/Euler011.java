...
public class Euler011 {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < 20; i++) {
			for (int j = 0; j < 17; j++) {
				int product = data[i][j] * data[i][j + 1] * data[i][j + 2]
						* data[i][j + 3];
				if (product > highestProduct)
					highestProduct = product;
				if (product == 2038878340) {
		...
		for (int i = 0; i < 20; i++) {
			for (int j = 0; j < 17; j++) {
				int product = data[j][i] * data[j + 1][i] * data[j + 2][i]
						* data[j + 3][i];
				if (product > highestProduct)
					highestProduct = product;
			}
		...
		for (int i = 0; i < 17; i++) {
			for (int j = 0; j < 17; j++) {
				int product = data[j][i] * data[j + 1][i + 1]
