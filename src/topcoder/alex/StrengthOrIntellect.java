...
public class StrengthOrIntellect {
	...
	public int numberOfMissions(final int[] strength, final int[] intellect,
			final int[] points) {
		for (int i = 0; i <= 1000; i++) {
			for (int j = 0; j <= 1000; j++) {
				...
				preComp[i][j] = new Pair(extraPoints, extraPoints < 0 ? 0
						: missions);
			}
		}
	...
	int func(final int strength, final int intellect) {
		...
		int res = (extra <= 0 ? memo[strength][intellect] : //
				Math.max(func(strength + 1, intellect),
